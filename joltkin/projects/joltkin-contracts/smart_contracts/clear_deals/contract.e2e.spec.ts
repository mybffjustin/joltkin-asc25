...
describe('ClearDeals contract', () => {
  ...
  const deploy = async (account: Address) => {
    const factory = localnet.algorand.client.getTypedAppFactory(ClearDealsFactory, {
  ...
  expect(result.return).toBe('Clear deals ready for World')
  })
  ...
  expect(result.returns[0]).toBe('Clear deals ready for World')
  expect(result.returns[1]).toBe('Clear deals ready for Jane')
