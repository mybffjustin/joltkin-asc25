...
const ConnectWallet = ({ openModal, closeModal }: ConnectWalletInterface) => {
  ...
  return (
    <dialog id="connect_wallet_modal" className={`modal modal-bottom sm:modal-middle backdrop-blur-sm ${openModal ? 'modal-open' : ''}`}>
            ...
            wallets?.map((wallet) => (
              <button
                ...
                className={`
                  ...
                  bg-neutral-700 hover:bg-neutral-600 border border-clear
              ...
              >
                ...
                <span className="font-semibold text-lg flex-1 text-left">{isKmd(wallet) ? 'LocalNet Wallet' : wallet.metadata.name}</span>
                {wallet.isActive && <BsCheckCircleFill className="text-xl text-cyan-400" />}
              </button>
            ))}
        </div>
